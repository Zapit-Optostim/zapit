function [calibratedPoints,rotMat] = rotateAndScaleCoords(template, refPointsStereotaxic, refPointsSample)
    ...
    %
    % Purpose
    % Calculates scale and rotation between the sample coords (refPointsSample) and
    % standard stereotaxic coords (refPointsStereotaxic). Applies the resulting transform
    % to the data in template. This allows us to map intended stimulation stereotaxic
    % coords onto the sample as imaged by the camera.
    %
    % Inputs
