function msg = returnMalformedKeyReport(~, fname)
    ...
    if length(uniqueInd) < length(ind)
        ...
        msg = sprintf(['file contains duplicate stimLocations key(s): %s. ', ...
            'Each condition must have a unique number, otherwise conditions are silently ', ...
            'lost when the file is read.'], mat2str(duplicated));
        return
    end
