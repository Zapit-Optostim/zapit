classdef APIWrapper < zapit.hardware.vidrio_daqmx.HasClassDataFile %& zapit.hardware.vidrio_daqmx.DClass
    ...
    properties (Constant, Hidden)
       apiDLLDefaultPath = fullfile(zapit.hardware.vidrio_daqmx.idioms.startPath,'windows','system32'); %This is default location for both Win32 and x64 platforms
    end
    ...
    methods
        function obj = APIWrapper()
            ...
            function ziniSetDefaultValues()
                ...
                defaultValMap('apiAuxFile1Paths') = zapit.hardware.vidrio_daqmx.util.className(obj,'classPrivatePath');
                defaultValMap('apiAuxFile2Names') = '';
                defaultValMap('apiAuxFile2Paths') = zapit.hardware.vidrio_daqmx.util.className(obj,'classPrivatePath');
                
                %Handle apiDLLPath default -- can be specified in MDF
                if ~isa(obj,'zapit.hardware.vidrio_daqmx.HasMachineDataFile') || ~isfield(obj.mdfData,'apiDLLPath')
                    defaultVal = '';
                else
                ...
                switch obj.apiHeaderRootPath
                    case 'class'
                        obj.apiHeaderRootPath = zapit.hardware.vidrio_daqmx.util.className(obj,'classPrivatePath');
                    case 'package'
                        obj.apiHeaderRootPath = zapit.hardware.vidrio_daqmx.util.className(obj,'packagePrivatePath');
                    otherwise
                        % "real path", no-op.
                end
            ...
            function ziniGetAPICurrentVersion()
                ...
                if isa(obj,'zapit.hardware.vidrio_daqmx.HasMachineDataFile')
    ...
    methods
        ...
        function val = get.apiDataFullFileName(obj)
            ...
            if isempty(obj.apiCachedDataPath)
                cachedDataPath = zapit.hardware.vidrio_daqmx.util.className(obj,'classPrivatePath');
            else
    ...
    methods (Hidden)
        ...
        function apiProcessErrorResponseCode(obj, responseCode, funcName)
            ...
            if warningOnly
                ...
                warning([zapit.hardware.vidrio_daqmx.util.className(obj) ':APICallWarning'],errorString);
                warning(s.state, 'backtrace');
            else
                ME = MException([zapit.hardware.vidrio_daqmx.util.className(obj) ':APICallError'],errorString);
                ME.throwAsCaller();
            end
    ...
    methods (Access=protected)
        ...
        function apiCurrentVersion = apiVersionDetect(obj)
            ...
            hPathChanger = zapit.hardware.vidrio_daqmx.util.PathChanger(obj.apiHeaderRootPath);
            ...
            try
                ...
                if ~libisloaded(obj.apiDLLName)
                    ...
                    elseif any(strcmpi(headerExt,{'.m' '.p'}))
                        try
                            hPathChanger = zapit.hardware.vidrio_daqmx.util.PathChanger(headerPath);
    ...
    methods (Static, Access=protected)
        ...
        function val = apiCachedDataPathStatic(className)
            ...
            if isempty(apiCachedDataPathSpec)
                val = zapit.hardware.vidrio_daqmx.util.className(className,'classPrivatePath');
            else
    ...
    methods (Static, Hidden)
        ...
        function showLoadLibraryWarnings()
            zapit.hardware.vidrio_daqmx.DClass.setClassDataVarStatic(mfilename('class'),'showLoadLibraryWarnings',true);
        end
        
        function hideLoadLibraryWarnings()
            zapit.hardware.vidrio_daqmx.DClass.setClassDataVarStatic(mfilename('class'),'showLoadLibraryWarnings',false);
        end
